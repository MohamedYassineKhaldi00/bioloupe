...
class MaterialService:
    ...
    async def create_material(self, data, user_id: str | uuid.UUID) -> Material:
        ...
        await self.activity.log(str(session_uuid), str(user_uuid), "created", "material", str(material.id))
        await self.db.flush()
        try:
    ...
    async def update_material(self, material_id: str | uuid.UUID, data, user_id: str | uuid.UUID) -> Material:
        ...
        await self.activity.log(str(material.session_id), str(self._as_uuid(user_id)), "updated", "material", str(material.id))
        await self.db.flush()
        await self.db.refresh(material)
        return material

    async def get_material(self, material_id: str | uuid.UUID) -> Material:
        material = await self.db.get(Material, self._as_uuid(material_id))
        if not material or material.deleted_at is not None:
            raise MaterialNotFound(material_id)
        return material

    async def soft_delete(self, material_id: str | uuid.UUID, user_id: str | uuid.UUID) -> None:
        material = await self.get_material(material_id)
        material.deleted_at = datetime.now(timezone.utc)
        await self.activity.log(str(material.session_id), str(self._as_uuid(user_id)), "deleted", "material", str(material.id))
        await self.db.flush()
    ...
    async def restore(self, material_id: str | uuid.UUID, user_id: str | uuid.UUID) -> Material:
        material = await self.get_material(material_id)
        ...
        await self.activity.log(str(material.session_id), str(self._as_uuid(user_id)), "restored", "material", str(material.id))
        await self.db.flush()
        return material
    ...
    async def list_materials_paginated(
        ...
        query = select(Material).where(Material.session_id == session_uuid, Material.deleted_at.is_(None))
        if material_type:
    ...
    async def search_materials(self, query: str, session_id: str | uuid.UUID | None, material_type: MaterialType | None):
        ...
        if session_id:
            stmt = stmt.where(Material.session_id == self._as_uuid(session_id))
        if material_type:
    ...
    async def add_tags(self, material_id: str | uuid.UUID, tags: list[str], user_id: str | uuid.UUID) -> Material:
        material = await self.get_material(material_id)
    ...
    async def remove_tags(self, material_id: str | uuid.UUID, tags: list[str], user_id: str | uuid.UUID) -> Material:
        material = await self.get_material(material_id)
        metadata = material.metadata_ or {}
        existing = set(metadata.get("tags", []))
    ...
    async def batch_update(self, updates: list, user_id: str | uuid.UUID) -> list[Material]:
    ...
    async def material_stats(self, session_id: str | uuid.UUID) -> dict:
        session_uuid = self._as_uuid(session_id)
        total = await self.db.execute(
            select(func.count(Material.id)).where(
                Material.session_id == session_uuid,
