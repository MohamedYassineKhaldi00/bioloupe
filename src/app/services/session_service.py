...
class SessionService:
    ...
    async def add_participant(self, session_id: str, user_id: str, permission: SessionPermission) -> SessionParticipant:
        result = await self.db.execute(
        ...
        if participant:
            ...
            await self.db.flush()
            await self.activity.log(session_id, user_id, "updated", "participant", user_id)
            return participant
        ...
        self.db.add(participant)
        await self.db.flush()
        await self.activity.log(session_id, user_id, "created", "participant", user_id)
        return participant

    async def update_participant(self, session_id: str, user_id: str, permission: SessionPermission) -> SessionParticipant:
        result = await self.db.execute(
            select(SessionParticipant).where(
                SessionParticipant.session_id == session_id,
                SessionParticipant.user_id == user_id,
    ...
    async def remove_participant(self, session_id: str, user_id: str) -> None:
        ...
        if participant:
            ...
            await self.activity.log(session_id, user_id, "deleted", "participant", user_id)

    async def list_participants(self, session_id: str) -> list[SessionParticipant]:
        result = await self.db.execute(
    ...
    async def search_sessions(self, query: str, team_id: str | None, include_archived: bool) -> list[Session]:
    ...
    async def list_tags(self, user_session_ids: list[str]) -> list[str]:
        if not user_session_ids:
            return []
        result = await self.db.execute(
            select(Session.topic_tags).where(Session.id.in_(user_session_ids))
        )
        tags: set[str] = set()
